...
app = Flask(__name__)
UPLOAD_FOLDER="D:/Flask Project/Signature App/static/images"
...
#     img = image.load_img(train_image_path+file_name, target_size=(IMG_WIDTH,IMG_HEIGHT,1), grayscale=True)
...
def home():
    if request.method == "POST":
        ...
        if image_file1 and image_file2:
            image_locaion = os.path.join(
                UPLOAD_FOLDER,image_file1.filename
            )
            image_file1.save(image_locaion)
            image_locaion = os.path.join(
                UPLOAD_FOLDER,image_file2.filename
            )
            image_file2.save(image_locaion)
...
if __name__ == '__main__':
    ...
    # model.init_from_file("D:\Flask Project\Signature App\model\model.pkl.gz")
    app.run(debug=True)
